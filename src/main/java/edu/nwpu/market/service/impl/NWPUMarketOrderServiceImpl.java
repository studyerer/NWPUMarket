...
public class NWPUMarketOrderServiceImpl implements NWPUMarketOrderService {
    ...
    @Override
    @Transactional
    public String updateOrderInfo(NWPUMarketOrder NWPUMarketOrder) {
        NWPUMarketOrder temp = NWPUMarketOrderMapper.selectByPrimaryKey(NWPUMarketOrder.getOrderId());
        //不为空且orderStatus>=0且状态为出库之前可以修改部分信息
        if (temp != null && temp.getOrderStatus() >= 0 && temp.getOrderStatus() < 3) {
            temp.setTotalPrice(NWPUMarketOrder.getTotalPrice());
            temp.setUserAddress(NWPUMarketOrder.getUserAddress());
