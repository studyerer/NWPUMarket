...
import edu.nwpu.market.controller.vo.SecondLevelCategoryVO;
import edu.nwpu.market.controller.vo.ThirdLevelCategoryVO;
import edu.nwpu.market.dao.GoodsCategoryMapper;
...
@Service
public class NWPUMarketlCategoryServiceImpl implements NWPUMarketCategoryService {
    ...
    public List<NWPUMarketIndexCategoryVO> getCategoriesForIndex() {
        ...
        if (!CollectionUtils.isEmpty(firstLevelCategories)) {
            ...
            //获取二级分类的数据
            List<GoodsCategory> secondLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(firstLevelCategoryIds, NWPUMarketCategoryLevelEnum.LEVEL_TWO.getLevel(), 0);
            if (!CollectionUtils.isEmpty(secondLevelCategories)) {
                List<Long> secondLevelCategoryIds = secondLevelCategories.stream().map(GoodsCategory::getCategoryId).collect(Collectors.toList());
                //获取三级分类的数据
                List<GoodsCategory> thirdLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(secondLevelCategoryIds, NWPUMarketCategoryLevelEnum.LEVEL_THREE.getLevel(), 0);
                if (!CollectionUtils.isEmpty(thirdLevelCategories)) {
                    ...
                    if (!CollectionUtils.isEmpty(secondLevelCategoryVOS)) {
                        ...
                        for (GoodsCategory firstCategory : firstLevelCategories) {
                            NWPUMarketIndexCategoryVO nwpuMarketIndexCategoryVO = new NWPUMarketIndexCategoryVO();
                            BeanUtil.copyProperties(firstCategory, nwpuMarketIndexCategoryVO);
                            //如果该一级分类下有数据则放入 NWPUMarketIndexCategoryVOS 对象中
    ...
    public SearchPageCategoryVO getCategoriesForSearch(Long categoryId) {
        ...
        if (thirdLevelGoodsCategory != null && thirdLevelGoodsCategory.getCategoryLevel() == NWPUMarketCategoryLevelEnum.LEVEL_THREE.getLevel()) {
            //获取当前三级分类的二级分类
            GoodsCategory secondLevelGoodsCategory = goodsCategoryMapper.selectByPrimaryKey(thirdLevelGoodsCategory.getParentId());
            if (secondLevelGoodsCategory != null && secondLevelGoodsCategory.getCategoryLevel() == NWPUMarketCategoryLevelEnum.LEVEL_TWO.getLevel()) {
                //获取当前二级分类下的三级分类List
                List<GoodsCategory> thirdLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(Collections.singletonList(secondLevelGoodsCategory.getCategoryId()), NWPUMarketCategoryLevelEnum.LEVEL_THREE.getLevel(), Constants.SEARCH_CATEGORY_NUMBER);
